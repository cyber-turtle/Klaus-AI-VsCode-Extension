...
export async function loadKlausRules(workspace: string) {
	try {
		const klausRules = await fs.readFile(
			path.join(workspace, ".klausrules"),
			"utf-8",
		);
		return klausRules;
	} catch (e) {
		console.error("Failed to load klaus rules", e);
	}
}
