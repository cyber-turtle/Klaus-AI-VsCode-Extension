...
import { loadKlausRules } from "./utils";
...
/**
 * KlausAgent - Autonomous coding assistant
 */
export class KlausAgent {
	...
	async initialize() {
		this.settings = await klausSettings.loadSettings();
		...
		this.aiProvider = await CreateAIProvider(this.settings, loggingProvider);
...
Default Shell: ${userInfo.shell}`;
		...
		const system = {
			role: "system",
			content: `You are an expert full stack developer collaborating with the user as their coding partner - you are Klaus.
This AI assistant was designed and developed by Maepl Labs.
...
- **Reflective Thinking**: For complex tasks, reflect on multiple possible sources of the problem or design approaches before implementation.
...
When debugging or implementing fixes, follow this senior engineering workflow:
1.  **Understand & Explore:** Familiarize yourself with the repository structure and the specific area of concern.
2.  **Reproduce:** When feasible, create a minimal script to reproduce the bug or verify a requirement. Execute it using the command tools.
3.  **Analyze & Hypothesize:** Formulate clear hypotheses about the root cause. Distill multiple possibilities down to the 1-2 most likely sources.
4.  **Validate:** Add logs or use inspection tools to validate your assumptions before writing the final code fix.
5.  **Fix & Verify:** Implement minimal, clean changes. Rerun your reproduction script or existing tests to confirm the fix.
6.  **Edge Cases:** Consider edge cases and verify that your solution handles them.
...
${this.workspace}`;
		...
		const rules = (await loadKlausRules(this.workspace)) ?? "";
