...
exports.PartitionedFileSystemSaver = void 0;
...
class PartitionedFileSystemSaver extends langgraph_checkpoint_1.BaseCheckpointSaver {
    /**
    ...
    ensureDirectories() {
        ...
        [this.baseDir, this.checkpointsDir, this.writesDir, this.dataDir].forEach((dir) => {
     ...
     */
    getThreadCheckpointsPath(threadId, checkpoint_ns = "") {
     ...
     */
    getThreadWritesPath(threadId, checkpoint_ns = "") {
        const hashedId = this.hashString(`${threadId}_${checkpoint_ns}`);
     ...
     */
    loadThread(threadId, checkpoint_ns = "") {
        const threadKey = `${threadId}:${checkpoint_ns}`;
        if (this.loadedThreads.has(threadKey))
            return;
        this.loadIndex();
        ...
        // Load checkpoints
        const checkpointsPath = this.getThreadCheckpointsPath(threadId, checkpoint_ns);
        try {
            ...
            console.warn(`Error loading checkpoints for thread ${threadKey}: ${error}`);
            this.threadMap.set(threadKey, new Map());
        }
        // Load writes
        ...
        try {
            if (fs.existsSync(writesPath)) {
                ...
                const writes = JSON.parse(writesData);
     ...
     */
    saveThread(threadId, checkpoint_ns = "") {
        const threadKey = `${threadId}:${checkpoint_ns}`;
        if (!this.dirtyThreads.has(threadKey))
            return;
        this.loadIndex();
        ...
        if (threadCheckpoints) {
            const checkpointsPath = this.getThreadCheckpointsPath(threadId, checkpoint_ns);
            try {
                const checkpointsData = JSON.stringify(Object.fromEntries(threadCheckpoints));
                fs.writeFileSync(checkpointsPath, checkpointsData, "utf8");
            }
            catch (error) {
                console.error(`Error saving checkpoints for thread ${threadKey}: ${error}`);
            }
        }
     ...
     */
    deserializeData(data, is_binary = false, is_file = false) {
        if (is_file) {
     ...
     */
    makeCheckpointKey(thread_id, checkpoint_ns, checkpoint_id) {
        return `${thread_id}:${checkpoint_ns}:${checkpoint_id}`;
    }
    /**
     * Creates a unique key for writes
     */
    makeWritesKey(thread_id, checkpoint_ns, checkpoint_id) {
        return `${thread_id}:${checkpoint_ns}:${checkpoint_id}`;
    }
    /**
    ...
    async getTuple(config) {
        ...
        if (checkpoint_id) {
            ...
            const sortedCheckpoints = Array.from(threadCheckpoints.values()).sort((a, b) => {
        ...
        const pendingWrites = await Promise.all(checkpointWrites.map(async (write) => {
            const deserializedValue = this.deserializeData(write.value, write.is_binary, write.is_file);
            return [
                write.task_id,
                write.channel,
        ...
        if (record.parent_checkpoint_id) {
            const parentWritesKey = this.makeWritesKey(record.thread_id, record.checkpoint_ns, record.parent_checkpoint_id);
            const parentWrites = this.writeMap.get(parentWritesKey) || [];
            // Get task writes from parent
            const taskWrites = parentWrites.filter((write) => write.channel === langgraph_checkpoint_1.TASKS);
            // Sort by index
            taskWrites.sort((a, b) => a.idx - b.idx);
            // Process the sends
            for (const send of taskWrites) {
                const deserializedValue = this.deserializeData(send.value, send.is_binary, send.is_file);
                pendingSends.push(await this.serde.loadsTyped(send.type ?? "json", deserializedValue));
            }
        }
        try {
            // Deserialize checkpoint and add pending_sends
            const deserializedCheckpoint = this.deserializeData(record.checkpoint, record.is_binary, record.is_file);
            const checkpointObj = {
            ...
            const metadataObj = (await this.serde.loadsTyped(record.type ?? "json", deserializedMetadata));
    ...
    async *list(config, options) {
        ...
        if (Object.keys(sanitizedFilter).length > 0) {
            ...
            for (const record of matches) {
                const deserializedMetadata = this.deserializeData(record.metadata, record.is_binary);
                const metadata = (await this.serde.loadsTyped(record.type ?? "json", deserializedMetadata));
                let includeRecord = true;
                for (const [key, value] of Object.entries(sanitizedFilter)) {
                    if (JSON.stringify(metadata[key]) !==
    ...
    async put(config, checkpoint, metadata) {
        ...
        const preparedCheckpoint = (0, langgraph_checkpoint_1.copyCheckpoint)(checkpoint);
        ...
        const { value: checkpointValue, is_binary: checkpointIsBinary, is_file: checkpointIsFile, } = this.serializeData(serializedCheckpoint, `${dataId}_checkpoint`);
        // Metadata is usually small, but handle it the same way
        const { value: metadataValue, is_binary: metadataIsBinary, is_file: metadataIsFile, } = this.serializeData(serializedMetadata, `${dataId}_metadata`);
    ...
    async putWrites(config, writes, taskId) {
        ...
        for (let idx = 0; idx < writes.length; idx++) {
            ...
            const { value, is_binary, is_file } = this.serializeData(serializedWrite, writeId);
            writeRecords.push({
                thread_id,
                checkpoint_ns,
                checkpoint_id,
     ...
     */
    removeThread(threadId, checkpoint_ns) {
        this.loadIndex();
        const threadPrefix = checkpoint_ns !== undefined
            ? `${threadId}:${checkpoint_ns}`
            : `${threadId}:`;
        // Get all thread keys to remove
        const threadsToRemove = this.index.threads.filter((t) => t.startsWith(threadPrefix));
    ...
    async delete(config) {
        ...
        if (checkpoint_id) {
            ...
            if (record.is_file && fs.existsSync(record.checkpoint)) {
                try {
                    ...
                    console.warn(`Error removing checkpoint file ${record.checkpoint}: ${error}`);
                }
            }
            // Delete associated writes
            const writesKey = this.makeWritesKey(thread_id, checkpoint_ns, checkpoint_id);
                ...
                for (const write of writes) {
                    if (write.is_file && fs.existsSync(write.value)) {
                        try {
                            ...
                            console.warn(`Error removing write file ${write.value}: ${error}`);
                        }
                    }
                }
        ...
        try {
            ...
            const checkpointsPath = this.getThreadCheckpointsPath(thread_id, checkpoint_ns);
     ...
     */
    cleanup(maxAge, maxCheckpointsPerThread, threadId) {
        this.loadIndex();
        ...
        if (threadId) {
            ...
            threadsToProcess = this.index.threads.filter((t) => t.startsWith(`${threadId}:`));
        }
        else {
        ...
        for (const threadKey of threadsToProcess) {
            ...
            if (maxCheckpointsPerThread !== undefined &&
            ...
            for (const checkpoint_id of checkpointsToRemove) {
                const key = this.makeCheckpointKey(currentThreadId, checkpoint_ns, checkpoint_id);
                const record = threadCheckpoints.get(key);
                if (record) {
                    ...
                    const writesKey = this.makeWritesKey(currentThreadId, checkpoint_ns, checkpoint_id);
                    const writes = this.writeMap.get(writesKey);
                    if (writes) {
...
exports.PartitionedFileSystemSaver = PartitionedFileSystemSaver;
