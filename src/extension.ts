...
export async function activate(context: vscode.ExtensionContext) {
	if (
		...
		vscode.window.showInformationMessage(
			"Klaus requires an open workspace to function.",
		);
		return;
	}
	...
	if (klausSettings.isDefault) {
		const result = await vscode.window.showErrorMessage(
			"Klaus has not yet been configured.",
			"Open Settings",
		);
	...
	klausSettings.registerOnChangeHandler((settings) => {
		if (chatViewProvider) {
