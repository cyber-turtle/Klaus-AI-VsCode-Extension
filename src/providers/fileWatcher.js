...
exports.WingmanFileWatcher = void 0;
const vscode = __importStar(require("vscode"));
const fs = __importStar(require("node:fs"));
const path = __importStar(require("node:path"));
const ignore_1 = __importDefault(require("ignore"));
const minimatch_1 = require("minimatch");
class WingmanFileWatcher {
    constructor(lspClient) {
        ...
        this.debounceTimeMs = 800; // Time to wait before indexing a changed file
        this.isReindexing = false;
        this.gitignoreMap = new Map();
    ...
    setupGitignoreWatchers() {
        ...
        this.gitignoreWatcher.onDidDelete((uri) => {
            ...
            const defaultIgnore = (0, ignore_1.default)().add(this.defaultIgnorePatterns);
     ...
     */
    updateGitignoreForFolder(folderPath) {
        try {
            const gitignorePath = path.join(folderPath, ".gitignore");
            if (fs.existsSync(gitignorePath)) {
                const gitignoreContent = fs.readFileSync(gitignorePath, "utf8");
                const ignoreInstance = (0, ignore_1.default)().add(this.defaultIgnorePatterns);
     ...
     */
    getIgnoreInstanceForFile(filePath) {
        const workspaceFolders = vscode.workspace.workspaceFolders;
        if (!workspaceFolders)
            return null;
    ...
    shouldIgnoreFile(filePath) {
        ...
        if (this.inclusionFilter && this.inclusionFilter.trim() !== "") {
            ...
            const shouldInclude = (0, minimatch_1.minimatch)(relativePath, this.inclusionFilter);
            if (!shouldInclude) {
    ...
    setupGitBranchWatcher() {
        ...
        for (const folder of workspaceFolders) {
            try {
                ...
                console.error(`Failed to set up Git branch watcher for ${folder.name}:`, error);
            }
        }
    }
    /**
     ...
     */
    handleGitBranchSwitch(folderPath) {
        ...
        this.branchSwitchDebounceTimer = setTimeout(() => {
            if (this.isReindexing)
                return;
            console.log(`Git branch switch detected in ${folderPath}, re-indexing...`);
            // Display notification to user
            vscode.window.showInformationMessage("Git branch switched - Wingman is re-indexing your files");
     ...
     */
    clearFolderFromIndex(folderPath) {
        ...
        this.indexingQueue = new Set(Array.from(this.indexingQueue).filter((file) => !file.startsWith(folderPath)));
    }
    /**
     ...
     */
    queueFolderForIndexing(folderPath) {
     ...
     */
    async startProgressiveIndexing(workspaceFolders, progress) {
     ...
     */
    async initialize(inclusionFilter) {
...
exports.WingmanFileWatcher = WingmanFileWatcher;
