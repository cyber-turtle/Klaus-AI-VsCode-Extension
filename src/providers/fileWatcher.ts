...
export class KlausFileWatcher {
	...
	private shouldIndexFile(filePath: string): boolean {
		try {
			...
			// Check gitignore and klausignore rules
	...
	private handleGitBranchSwitch(folderPath: string): void {
		...
		this.branchSwitchDebounceTimer = setTimeout(() => {
			...
			vscode.window.showInformationMessage(
				"Git branch switched - Klaus is re-indexing your files",
			);
		...
		vscode.window.withProgress(
			{
				...
				title: "Klaus: Indexing workspace files",
				cancellable: true,
			},
