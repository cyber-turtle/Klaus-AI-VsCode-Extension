...
export class ConfigViewProvider implements vscode.WebviewViewProvider {
	public static readonly viewType = "klaus.configview";
	public static readonly showConfigCommand = "klausai.openconfig";
	...
	public openInPanel() {
		...
		settingsPanel.webview.onDidReceiveMessage(async (data: AppMessage) => {
			...
			switch (command) {
				...
				case "saveSettings":
					await klausSettings.saveSettings(value as Settings);
					try {
						...
						if (!this._lspClient.isRunning()) {
							await this._lspClient.activate(
								this.context,
								await klausSettings.loadSettings(),
							);
						}
				...
				case "load-lmstudio-models": {
					const initSettings = await klausSettings.loadSettings();
