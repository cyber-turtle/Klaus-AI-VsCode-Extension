...
export class RefactorProvider implements vscode.CodeActionProvider {
	public static readonly command = "klausai.refactorcode";
	...
	async provideCodeActions(
		...
		const codeAction = new vscode.CodeAction(
			"🎄 Refactor using Klaus",
			vscode.CodeActionKind.Refactor,
		);
		codeAction.edit = new vscode.WorkspaceEdit();
		codeAction.command = {
			command: RefactorProvider.command,
			title: "🎄 Refactor using Klaus",
			arguments: [
				document,
				range,
				CreateAIProvider(await klausSettings.loadSettings(), loggingProvider),
