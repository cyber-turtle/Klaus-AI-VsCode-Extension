...
import { KlausAgent } from "../composer";
...
export class LSPServer {
	...
	private postInitialize = async () => {
		...
		loggingProvider.logInfo(
			`Klaus LSP initialized for workspace: ${workspaceFolder}`,
		);
	...
	private compose = async (
		...
		try {
			...
			for await (const event of this.composer.execute(
				...
				await this.connection?.sendRequest("klaus/compose", event);
...
${input}`,
	...
	private addEvents = async () => {
		...
		this.connection?.onRequest("klaus/updateSettings", async () => {
			const settings = await klausSettings.loadSettings(true);
			await this.composer?.initialize();
		...
		this.connection?.onRequest("klaus/getIndexedFiles", async () => {
			if (!this.vectorStore) return;

			return this.vectorStore.getIndexedFiles();
		});

		this.connection?.onRequest("klaus/resyncIndex", async () => {
			if (!this.vectorStore) return;
