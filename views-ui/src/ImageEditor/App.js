...
const fabricModule = __importStar(require("fabric"));
require("./App.css");
...
function App() {
    ...
    const fabricCanvasRef = (0, react_1.useRef)(null);
    const [fabricCanvas, setFabricCanvas] = (0, react_1.useState)(null);
    const [activeTool, setActiveTool] = (0, react_1.useState)('pen');
    const [brushColor, setBrushColor] = (0, react_1.useState)('#000000');
    const [brushSize, setBrushSize] = (0, react_1.useState)(5);
    ...
    const [showColorWheel, setShowColorWheel] = (0, react_1.useState)(false);
    const [showBrushSize, setShowBrushSize] = (0, react_1.useState)(false);
    const [recentColors, setRecentColors] = (0, react_1.useState)(['#000000']);
    const [zoom, setZoom] = (0, react_1.useState)(1);
    const [isPanning, setIsPanning] = (0, react_1.useState)(false);
    const [lastPanPosition, setLastPanPosition] = (0, react_1.useState)(null);
    const [generating, setGenerating] = (0, react_1.useState)(false);
    // New state for generate instructions input
    const [showGenerateInput, setShowGenerateInput] = (0, react_1.useState)(false);
    const [generateInstructions, setGenerateInstructions] = (0, react_1.useState)("");
    ...
    const saveToHistory = (0, react_1.useCallback)(() => {
        if (!fabricCanvas)
            return;
        const currentState = {
    ...
    const handleCanvasMouseDown = (event) => {
        ...
        if (event.button !== 0)
            return;
        if (!fabricCanvas || (activeTool !== 'rectangle' && activeTool !== 'circle' && activeTool !== 'text'))
            return;
    ...
    const handleCanvasMouseMove = (event) => {
        if (!fabricCanvas || !isDrawing || (activeTool !== 'rectangle' && activeTool !== 'circle'))
            return;
        const coords = getCanvasCoordinates(event);
        if (!coords)
            return;
    ...
    const handleGenerationSubmit = async () => {
        if (!fabricCanvas || !generateInstructions.trim())
            return;
    ...
    const handleImageUpload = (event) => {
        if (!fabricCanvas || !event.target.files || event.target.files.length === 0)
            return;
    ...
    const handleSaveAs = async () => {
        ...
        vscode_1.vscode.postMessage({ command: 'save-object-image', value: dataURL });
    ...
    return (<div className="app-container">
			<div className="paint-app" onMouseDown={handlePanStart} onMouseMove={handlePanMove} onMouseUp={handlePanEnd} onMouseLeave={handlePanEnd}>
				{/* Floating Tools Panel */}
				<div className="floating-tools">
					<button type="button" className={`tool-btn ${activeTool === 'select' ? 'active' : ''}`} onClick={() => handleToolChange('select')} title="Select (S)">
						<i className="icon select-icon"><hi2_1.HiCursorArrowRays size={20}/></i>
					</button>
					<button type="button" className={`tool-btn ${activeTool === 'pen' ? 'active' : ''}`} onClick={() => handleToolChange('pen')} title="Pen (P)">
						<i className="icon pen-icon"><fa6_1.FaPencil size={16}/></i>
					</button>
					<button type="button" className={`tool-btn ${activeTool === 'rectangle' ? 'active' : ''}`} onClick={() => handleToolChange('rectangle')} title="Rectangle (R)">
						<i className="icon rectangle-icon"><fa6_2.FaRegSquare size={16}/></i>
					</button>
					<button type="button" className={`tool-btn ${activeTool === 'circle' ? 'active' : ''}`} onClick={() => handleToolChange('circle')} title="Circle (C)">
						<i className="icon circle-icon"><fa_1.FaRegCircle size={16}/></i>
					</button>
					<button type="button" className={`tool-btn ${activeTool === 'text' ? 'active' : ''}`} onClick={() => handleToolChange('text')} title="Text (T)">
						<i className="icon text-icon"><io5_1.IoText size={16}/></i>
					</button>
					<button type="button" className={`tool-btn ${activeTool === 'eraser' ? 'active' : ''}`} onClick={() => handleToolChange('eraser')} title="Eraser (E)">
						<i className="icon eraser-icon"><fa_2.FaEraser size={16}/></i>
					</button>
					<div className="flex flex-col">
						<button type="button" className={`tool-btn ${showColorWheel ? 'active' : ''}`} onClick={() => {
            setShowColorWheel(!showColorWheel);
            setShowBrushSize(false);
        }} title="Color">
							<i className="icon"><vsc_1.VscSymbolColor size={16}/></i>
						</button>
						<span style={{ borderBottom: `1px solid ${brushColor}` }}/>
					</div>
					<button type="button" className={`tool-btn ${showBrushSize ? 'active' : ''}`} onClick={() => {
            setShowBrushSize(!showBrushSize);
            setShowColorWheel(false);
        }} title="Brush Size">
						<i className="icon"><cg_1.CgSize size={16}/></i>
					</button>
				</div>
				...
				<div className="floating-actions">
					<div className="flex gap-4">
						<button type="button" className="action-btn" onClick={handleClear} title="Clear Canvas">
							Clear
						</button>
						...
						<button type="button" className="action-btn flex flex-row items-center gap-2" onClick={handleGenerateClick} title="Generate Image" disabled={generating}>
							{generating && (<ai_1.AiOutlineLoading3Quarters className="animate-spin text-stone-400" size={16}/>)}
							Generate

						</button>
					</div>
					{/* Generate Instructions Input Box */}
					{showGenerateInput && (<div className="generate-input-container mt-4">
							<div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 p-4">
								...
								<textarea value={generateInstructions} placeholder="Describe what you want to generate..." onChange={(e) => setGenerateInstructions(e.target.value)} onKeyDown={(e) => {
                if (e.key === 'Enter' && !e.shiftKey) {
                    e.preventDefault();
                   ...
                   min-h-[100px] transition-colors duration-200"/>
								<div className="space-y-3">
									...
									<div className="flex justify-end gap-3">
										<button type="button" className="px-4 py-2 rounded-md text-gray-700 dark:text-gray-200 
                       bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 
                       dark:hover:bg-gray-600 transition-colors duration-200
                       border border-gray-300 dark:border-gray-600" onClick={() => setShowGenerateInput(false)}>
											Cancel
										</button>
										<button type="button" className={`px-4 py-2 rounded-md text-white
                       bg-blue-600 hover:bg-blue-700 
                       disabled:opacity-50 disabled:cursor-not-allowed
                       transition-colors duration-200
                       ${!generateInstructions.trim() ? 'opacity-50 cursor-not-allowed' : ''}`} onClick={handleGenerationSubmit} disabled={!generateInstructions.trim()}>
											Generate
										</button>
									</div>
								</div>
				...
				<div className={`color-wheel-container ${showColorWheel ? 'visible' : ''}`}>
					<input type="color" value={brushColor} onChange={(e) => handleColorChange(e.target.value)} className="color-wheel"/>
					<div className="recent-colors">
						{recentColors.map((color) => (
        // biome-ignore lint/a11y/useKeyWithClickEvents: <explanation>
        <div key={color} className={`recent-color ${brushColor === color ? 'active' : ''}`} style={{ backgroundColor: color }} onClick={() => handleColorChange(color)} title={color}/>))}
					</div>
				</div>

				{/* Brush Size Controls */}
				<div className={`brush-size-container ${showBrushSize ? 'visible' : ''}`}>
					<input type="range" min="1" max="50" value={brushSize} onChange={(e) => handleBrushSizeChange(Number(e.target.value))} className="brush-size-slider"/>
					<div className="brush-size-preview">
						<div className="brush-size-circle" style={{
            width: `${brushSize}px`,
            height: `${brushSize}px`,
				...
				<div className="zoom-controls p-2">
					<div className="flex items-center">
						<span data-tooltip-id="tips">
							<fa_1.FaQuestionCircle size={16}/>
							<react_tooltip_1.Tooltip id="tips" place="bottom" content="You can save or generate based off a specific shape by selecting it. If no selection is made, it will use the canvas's viewport."/>
						</span>
					</div>
					...
					<button type="button" className="zoom-btn" onClick={handleResetZoom} title="Reset Zoom">
						1:1
					</button>
				</div>

				{/* Canvas Container */}
				<div ref={canvasContainerRef} className="canvas-container" onMouseDown={handleCanvasMouseDown} onMouseMove={handleCanvasMouseMove} onMouseUp={handleCanvasMouseUp} onMouseLeave={handleCanvasMouseUp}>
					...
					{contextMenu.visible && (
        // biome-ignore lint/a11y/useKeyWithClickEvents: <explanation>
        <div className="context-menu" style={{ position: 'absolute', left: contextMenu.x, top: contextMenu.y, background: '#fff', border: '1px solid #ccc', borderRadius: '4px', zIndex: 1000, padding: '4px' }} onClick={(e) => e.stopPropagation()} // Prevent click from bubbling up
        >
