...
import { TbNeedleThread, TbChevronDown, TbGitBranch } from "react-icons/tb";
...
*/
  ThreadManagement = ({ loading }: { loading: boolean }) => {
    ...
    return (
      <div className='flex align-middle items-center gap-1 pl-2'>
        <div className="relative" ref={dropdownRef}>
          <button
            ...
            className={`
              ...
              bg-[var(--vscode-editor-inactiveSelectionBackground)]/20
              border border-[var(--vscode-widget-border)]/50
              hover:bg-[var(--vscode-toolbar-hoverBackground)]
              hover:border-[var(--vscode-focusBorder)]/30
          ...
          >
            {loading ? (
                 <AiOutlineLoading3Quarters className="animate-spin text-[var(--vscode-textLink-foreground)]" size={14} />
            ) : (
                <div className={`p-1 rounded-full bg-[var(--vscode-textLink-foreground)]/10 text-[var(--vscode-textLink-foreground)]`}>
                  <TbNeedleThread size={14} />
                </div>
            )}
            
            <span className="truncate text-sm font-medium text-[var(--vscode-foreground)] opacity-90 group-hover:opacity-100 transition-opacity">
              {activeThread?.title || 'New Thread'}
            </span>
            
            <TbChevronDown 
              size={14} 
              className={`text-[var(--vscode-descriptionForeground)] transition-transform duration-300 ${isDropdownOpen ? 'rotate-180' : ''}`} 
            />
          </button>
        ...
        <button
          ...
          className="ml-1 p-1.5 rounded-full hover:bg-[var(--vscode-toolbar-hoverBackground)] text-[var(--vscode-descriptionForeground)] hover:text-[var(--vscode-foreground)] transition-all"
          aria-label="Visualize threads"
          title="Visualize Threads"
        >
          <TbGitBranch size={18} className="transform rotate-90" />
        </button>
      </div>
    );
  };
