...
exports.FileDropdown = void 0;
const react_1 = require("react");
const FileDropdown = ({ isLightTheme, showDropdown, dropdownItems, focusedDropdownIndex, onSelect, }) => {
    ...
    return (<div ref={dropdownRef} className={`absolute bottom-[8rem] mb-1 left-0 w-full z-20 ${dropdownClasses} border rounded overflow-y-auto max-h-[512px] shadow-lg`}>
			{dropdownItems.map((item, index) => (<div 
        // biome-ignore lint/suspicious/noArrayIndexKey: <explanation>
        key={index} className={`${dropdownItemClasses} ${index === focusedDropdownIndex ? selectedItemClasses : ''}`} onClick={() => onSelect(item)} onKeyDown={(e) => {
...
//# sourceMappingURL=FileDropdown.js.map